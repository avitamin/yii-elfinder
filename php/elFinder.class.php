...
class elFinder
{
    ...
    protected $_options = array(
        ...
        'tmbCrop' => true,
        // crop thumbnails (true - crop, false - scale image to fit thumbnail size)
        'tmbBgColor' => '#ffffff',
    ...
    protected function _dirSize($path)
    {
        ...
        if ($this->_options['du']) {
            ...
            for ($i = 0; $i < count($ls); $i++) {
                if ($this->_isAccepted($ls[$i])) {
                    $p = $path . DIRECTORY_SEPARATOR . $ls[$i];
                    $size += filetype($p) == 'dir' && $this->_isAllowed(
                        $p,
                        'read'
                    ) ? $this->_dirSize($p) : filesize($p);
                }
            }
        }
        return $size;
